...
namespace BibliothèqueSqlite.Forms
{
    public partial class FrmListBooks : Form
    {
        ...
        private void RefreshData()
        {
            ...
            {
                dataGridView1.DataSource = (from book in db.tblBooks
                                            from Publ in db.tblPublishing.Where(p => p.Id == book.idPublishing )
                                            from clsbook in db.tblPublishing.Where(c => c.Id == book.idClassBook)
                                            select new
                                            {
                                                book.Id,
                                                book.dateBook,
                                                ...
                                                book.NmbCopieBook,
                                                book.QteBook,
                                                QuantityAvailable = (book.QteBook)-db.tblLoan.Count(l=>l.idBook==book.Id && l.dateActualEnty==null),
                                                book.PriceBook,
                                                Publ.Publishing,
        ...
        private void serchBook(string titel)
        {
            ...
            using (var db = new DBContext())
            {
                dataGridView1.DataSource = (from book in db.tblBooks.Where(b => b.TitalBook.Contains(titel)||b.AuthorName.Contains(titel))
                                            from Publ in db.tblPublishing.Where(p => p.Id == book.idPublishing)
                                            from clsbook in db.tblPublishing.Where(c => c.Id == book.idClassBook)
                                            select new
                                            {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            using (var db = new DBContext())
            {
                ...
                {
                    db.tblBooks.Remove(db.tblBooks.Single(x => x.Id == id));
                    db.SaveChanges();
