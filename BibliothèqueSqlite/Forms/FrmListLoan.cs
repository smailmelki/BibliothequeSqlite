...
namespace BibliothèqueSqlite.Forms
{
    public partial class FrmListLoan : Form
    {
        ...
        private void RefreshData()
        {
            ...
            {
                dataGridView1.DataSource = (from Loan in db.tblLoan
                                            from book in db.tblBooks.Where(b => b.Id == Loan.idBook)
                                            from Stud in db.tblStudents.Where(p => p.Id == Loan.idStudents)
                                            select new
                                            {
                                                Loan.Id,
                                                Loan.idBook,
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            using (var db = new DBContext())
            {
                ...
                {
                    db.tblLoan.Remove(db.tblLoan.Single(x => x.Id == id));
                    db.SaveChanges();
        ...
        private void radioButton2_CheckedChanged(object sender, EventArgs e)
        {
            using (var db = new DBContext())
            {
                var data = (from Loan in db.tblLoan
                                            from book in db.tblBooks.Where(b => b.Id == Loan.idBook)
                                            from Stud in db.tblStudents.Where(p => p.Id == Loan.idStudents)
                                            select new
                                            {
